...
const UI = (() => {
  ...
  /* ---------------- Sidebar / mobile nav ---------------- */
  const NAV_ITEMS = [
    { key: 'home', href: '#home', icon: Icons.home, label: 'Home' },
    { key: 'search', href: '#search', icon: Icons.search, label: 'Search' },
    { key: 'favorites', href: '#favorites', icon: Icons.heartOutline, label: 'Favorites' },
    { key: 'recently-played', href: '#recently-played', icon: Icons.clock, label: 'Recently Played' },
    { key: 'radio', href: '#radio', icon: Icons.radio, label: 'Live Radio' },
    { key: 'settings', href: '#settings', icon: Icons.settings, label: 'Settings' },
  ];

  function renderPrimaryNav(activeKey = 'home') {
    const nav = document.getElementById('primary-nav');
    if (!nav) return;
    nav.innerHTML = NAV_ITEMS.map((item) => `
      <a href="${item.href}" class="nav-item${item.key === activeKey ? ' active' : ''}" data-route="${item.key}">
        ${item.icon}<span>${item.label}</span>
      </a>
    `).join('');
  }

  function setActiveNav(activeKey = 'home') {
    const nav = document.getElementById('primary-nav');
    if (!nav) return;
    nav.querySelectorAll('.nav-item').forEach((item) => {
      const route = item.getAttribute('data-route') || parseRouteKey(item.getAttribute('href'));
      const match = route === activeKey;
      item.classList.toggle('active', match);
    });
  }

  function parseRouteKey(href) {
    if (!href) return 'home';
